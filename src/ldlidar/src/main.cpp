...
int main(int argc, char **argv)
{
    ...
    nh_private.getParam("flag_parted", flag_parted);
    nh_private.param<std::vector<int>>("disable_min", disable_angle_min_range, {0});
    nh_private.param<std::vector<int>>("disable_max", disable_angle_max_range, {0});
    for (long unsigned int j = 0; j < disable_angle_min_range.size(); ++j)
    {
        int change_angle = 360 - disable_angle_min_range[j];
    ...
    if (product_name == "LDLiDAR_LD14P")
    {
        ROS_INFO("LDLiDAR_LD14P");
        baudrate = 115200;
            ...
            if (flag_parted)
            {
                if (disable_angle_min_range.size() == disable_angle_max_range.size())
                {
                    for (long unsigned int j = 0; j < disable_angle_min_range.size(); ++j)
                    {
                        ...
                        {
                            if ((dir_angle < (disable_angle_max_range[j])) || (dir_angle >= (disable_angle_min_range[j])))
                            {
                                range = std::numeric_limits<float>::infinity(); // 如果不在角度不在扫描范围内，将该角度对应的目标距离置为无穷大
                                intensity = 0;
                            }
                        }
                        else
                        {
                            if ((dir_angle > (disable_angle_min_range[j])) && (dir_angle <= (disable_angle_max_range[j])))
                            {
                                range = std::numeric_limits<float>::infinity(); // 如果不在角度不在扫描范围内，将该角度对应的目标距离置为无穷大
                                intensity = 0;
                            }
